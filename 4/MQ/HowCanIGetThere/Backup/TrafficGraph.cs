...
namespace Graph
{
    abstract class TrafficGraph<T> : Graph<T>
    {
        ...
        protected Dictionary<string, CommunicationSource> avalibleCommunicationSources = new Dictionary<string, CommunicationSource>(); // 
        ...
        {
            var startStreetNode = findParticularNode(startStreetName);
            var endStretNode = findParticularNode(endStringName);
            return null;
        }

        protected TrafficGraph<T> findParticularNode(string street)
        {

            //var corectNode = (TrafficGraph<T>)node[street].findParticularNode(node.name);
            return null; //node;
        }
